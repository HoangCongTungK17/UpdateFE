...
const ChatBot = () => {
  ...
  const [messages, setMessages] = useState<IMessage[]>([
    {
      text: "Xin chào! Tôi là AI hỗ trợ tìm việc. Bạn cần tìm công việc gì?",
      sender: "bot",
    },
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `http://localhost:8080/api/v1/ai/chat?message=${encodeURIComponent(
          userMsg
        )}`
      );
      ...
      const botMsg =
        res && res.data && res.data.data
          ? res.data.data
          : "Xin lỗi, tôi không hiểu câu trả lời.";

      setMessages((prev) => [
        ...prev,
        {
          text: typeof botMsg === "string" ? botMsg : JSON.stringify(botMsg),
          sender: "bot",
        },
      ]);
    } catch (error) {
      setMessages((prev) => [
        ...prev,
        { text: "⚠️ Lỗi kết nối Server AI. Vui lòng thử lại!", sender: "bot" },
      ]);
      console.error(error);
    } finally {
      setIsLoading(false);

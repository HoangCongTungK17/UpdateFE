...
export const getLocationName = (value: string) => {
  ...
  // Tìm theo value code (HANOI, HOCHIMINH, etc.)
  const locationByValue = LOCATION_LIST.find((item) => item.value === value);
  if (locationByValue) return locationByValue.label;

  // Tìm theo label (Hà Nội, Hồ Chí Minh, etc.)
  const locationByLabel = LOCATION_LIST.find((item) => item.label === value);
  if (locationByLabel) return locationByLabel.label;

  // Nếu không tìm thấy trong danh sách, trả về giá trị gốc
  return value;
};
